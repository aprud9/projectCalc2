        ...
        {
            {
                ...
                Console.WriteLine("Введите выражение в формате 50+5, 96-2, 10*2, 500/5 и т.п. Пример выражения с дробным числом: 99,9*1: ");
                ...
                try 
                {
                    ...
                    for (int i = 1; i < countS; i++) // Начинаем цикл с i = 1 на случай, если первое число отрицательное
                    {
                        if (s[i] == '+' || s[i] == '-' || s[i] == '*' || s[i] == '/') 
                        {
                            ...
                            break; // Выходим из цикла сразу после определения знака для избежания лишней работы цикла и ошибки, если второе число будет отрицательное
                        }
                    }
                }
                catch
